...
<script lang="ts">
	...
	// Note: tappedId parameter makes the dependency explicit for Svelte's reactivity tracking
	function actionVis(msgId: string, tappedId: string | null): string {
		return (tappedId === msgId ? '' : 'opacity-0') + ' md:opacity-0 md:group-hover:opacity-100 focus:opacity-100';
	}
	...
	// Message grouping for side-by-side layout
...
<div bind:this={messagesContainer} on:scroll={handleScroll} role="log" aria-live="polite" aria-label="Chat messages" class="absolute inset-0 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4 mobile-scroll">
	...
	{#each groupedMessages as group (group.messages[0].id)}
		{#if group.type === 'user'}
			...
			<div class="flex justify-end" aria-label="You said">
				...
				>
					<div class="px-3 md:px-4 py-2.5 md:py-3 rounded-2xl md:rounded-lg bg-brand text-white rounded-br-sm md:rounded-br-none">
				...
				<div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mb-2">
					...
					{#if bot.webSearchEnabled}
						<span title="Web search enabled">
							<svg class="w-3.5 h-3.5 text-brand dark:text-brand-text" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
