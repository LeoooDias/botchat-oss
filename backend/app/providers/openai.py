...
#
# Purpose: Demonstrate OpenAI API privacy flags (no training opt-out)
#
...
def _convert_heic_to_jpeg(image_bytes: bytes) -> tuple[bytes, str]:
    ...
    try:
        ...
        logger.debug("HEIC->JPEG conversion: %d -> %d bytes", len(image_bytes), len(converted_bytes))
            ...
            for fd in file_data:
                ...
                if file_bytes and mime_type.startswith("image/"):
                    ...
                    if mime_type == "image/heic":
                        ...
                        logger.debug("Converted HEIC image for OpenAI compatibility")
